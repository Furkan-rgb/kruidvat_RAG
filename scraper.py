...
# Single source of truth for local Ollama defaults used by the CLI
OLLAMA_DEFAULT_MODEL = "ministral-3:3b"
...
async def main_async():
    ...
    parser.add_argument(
        "--category", default="https://www.kruidvat.nl/verzorging/haarstylingproducten"
    )
    parser.add_argument("--db", default="kruidvat.db")
    parser.add_argument("--headed", action="store_true")
    parser.add_argument("--concurrency", type=int, default=10)
    parser.add_argument("--max-pages", type=int, default=200)
    parser.add_argument("--delay", type=float, default=0.2)
    parser.add_argument(
        "--limit", type=int, default=None, help="Process at most N products"
    )
    ...
    parser.add_argument(
        "--ollama-timeout",
        type=float,
        default=OLLAMA_DEFAULT_TIMEOUT,
    ...
    async with async_playwright() as p:
        ...
        page_size_local = 100
        try:
            init_page = await open_page_and_prep(context, args.category)
        except Exception:
            init_page = await context.new_page()
            try:
                await init_page.goto(
                    args.category, wait_until="domcontentloaded", timeout=30000
                )
        ...
        # small helper to run prep actions after each paged goto (pager calls this with page)
        async def prep_page_after_goto(page):
            await click_cookie_if_present(page)
            await click_read_more(page)

        # 4) Collect product links by paginating the category using pager.collect_all_product_links
        ...
        # 5) Filter out already existing URLs and apply optional limit
